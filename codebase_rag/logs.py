...
EMBEDDING_RECONCILE_OK = "Embedding verification: all {count} expected embeddings found"
EMBEDDING_RECONCILE_MISSING = "Embedding verification: {missing} of {expected} embeddings missing (IDs: {sample_ids})"
...
CLASS_FOUND_CONTRACT = "  Found Contract: {name} (qn: {qn})"
CLASS_FOUND_LIBRARY = "  Found Library: {name} (qn: {qn})"
SOL_FOUND_EVENT = "  Found Event: {name} (qn: {qn})"
SOL_FOUND_MODIFIER = "  Found Modifier: {name} (qn: {qn})"
SOL_FOUND_STATE_VAR = "  Found StateVariable: {name} (qn: {qn})"
SOL_FOUND_CUSTOM_ERROR = "  Found CustomError: {name} (qn: {qn})"
...
MIGRATION_DRY_RUN_METHOD_EXPORTED = "[DRY RUN] Would set is_exported=false for {count} Method nodes"
