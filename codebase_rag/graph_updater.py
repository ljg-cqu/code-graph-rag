...
class GraphUpdater:
    ...
    def _process_files(
        self, force: bool = False, num_workers: int | None = None
    ) -> None:
        # Use configured default if no explicit worker count provided
        effective_num_workers = (
            num_workers
            if num_workers is not None
            else settings.PARALLEL_INDEXING_WORKERS
        )
        ...
        actual_workers = (
            min(effective_num_workers, available_cores, len(changed_files))
            if changed_files
            else 1
        )

        logger.info(
            f"Found {len(changed_files)} changed files to process with {actual_workers} parallel workers (round-robin distribution)"
        )
        if actual_workers < effective_num_workers:
            logger.debug(
